...
export default function AuthBoundary({children}){
 ...
 async function callback(url){
  ...
  try{
   ...
   const response=await fetch(info.token_endpoint,{method:'POST',headers:{'Content-Type':'application/x-www-form-urlencoded'},body:new URLSearchParams({grant_type:'authorization_code',client_id:import.meta.env.VITE_OIDC_CLIENT_ID,code:received.searchParams.get('code'),code_verifier:saved.verifier,redirect_uri:redirect()})});
 ...
 async function login(){setPending(true);setError('');try{
  const info=await discovery(),client=import.meta.env.VITE_OIDC_CLIENT_ID;
  ...
  sessionStorage.setItem('synex-pkce',JSON.stringify({verifier,state,created:Date.now()}));
